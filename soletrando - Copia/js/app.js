...
const App = {
    ...
    loadSelectionSeries() {
        ...
        if (series.length === 0) {
            container.innerHTML = `
                <div class="selection-empty">
                    ...
                    <a href="#" onclick="App.showScreen('admin'); return false;" class="selection-empty-link">${T('Ir para o Painel do Professor')}</a>
                </div>`;
            return;
        }
        ...
        container.innerHTML = series.map(s => {
            ...
            return `
                <div class="selection-item" data-id="${s.id}" onclick="App.selectSerie('${s.id}')">
    ...
    selectSerie(serieId) {
        ...
        if (turmas.length > 0) {
            ...
            container.innerHTML = turmas.map(t => `
                <div class="selection-item turma-multi-item" data-id="${t.id}" onclick="App.toggleTurma('${t.id}')">
    ...
    showDisciplinasStep() {
        ...
        container.innerHTML = disciplinasWithWords.map(d => {
            ...
            return `
                <div class="selection-item" data-id="${d.id}" onclick="App.toggleDisciplina('${d.id}')">
    ...
    renderPalavrasOptions(total) {
        ...
        container.innerHTML = options.map(v => {
            ...
            return `<button class="btn btn-secondary${active}" onclick="App.setPalavrasPorTurma('${v}')">${label}</button>`;
        }).join('');
...
loadResultsPage(eventoId, execucaoId, eventName) {
    ...
    autoEncerrarEventos() {
        ...
        if (backfill) {
            try { Data.saveEventos(eventos); } catch(e) {}
        }
    },
    ...
    loadHomeBanner() {
        ...
        doHoje.forEach(e => {
            ...
            <div class="home-event-card today">
                ...
                <div class="home-event-btns">
                    ${temLogs ? `<button class="btn btn-small home-event-results-btn" onclick="App.showEventResults('${e.id}')">
                    ...
                    <button class="btn btn-primary btn-small" onclick="App.startEventGame('${e.id}')">${T('Iniciar')}</button>
                </div>
            </div>`;
        });
        ...
        if (encerrados.length > 0) {
            html += `
            <button class="home-event-toggle" onclick="App.toggleHomeHistory()">
