...
// ===== CACHE DE TRADUÇÃO DE LOGS =====
// Evita reprocessar as mesmas strings de log repetidamente durante a renderização.
const _logDetailsCache = new Map();
const _LOG_DETAILS_CACHE_MAX = 500;

/**
 * Limpa o cache de tradução de logs (chamado ao trocar de idioma).
 */
function _clearLogDetailsCache() {
    _logDetailsCache.clear();
}
...
const I18n = {
    ...
    apply() {
        ...
        document.querySelectorAll('[data-i18n-title]').forEach(el => {
            const value = this.t(el.getAttribute('data-i18n-title'));
            el.title = value;
            // Botões só-ícone usam o mesmo texto do title como aria-label,
            // para leitores de tela (title sozinho não é confiável nesse caso).
    ...
    logDetails(str) {
        ...
        // Verifica cache
        const cacheKey = this.current + '|' + str;
        if (_logDetailsCache.has(cacheKey)) {
            return _logDetailsCache.get(cacheKey);
        }
        ...
        if (items.length > 1 && items.every(p => _isLogItem(v, p))) {
            result = _logItems(v, str);
        } else {
            result = str;
            for (const [re, fn] of v.rules) {
                const m = re.exec(str);
                if (m) { result = fn(m, v); break; }
            }
            if (result === str && _isLogItem(v, str)) {
                result = _logItem(v, str);
            } else if (result === str) {
                const cm = /^(.+): (.+) → (.+)$/.exec(str);
                if (cm) result = `${v.settings[cm[1]] || cm[1]}: ${_logVal(v, cm[2])} → ${_logVal(v, cm[3])}`;
            }
        }

        // Armazena no cache (LRU simples)
        if (_logDetailsCache.size >= _LOG_DETAILS_CACHE_MAX) {
            const firstKey = _logDetailsCache.keys().next().value;
            _logDetailsCache.delete(firstKey);
        }
        _logDetailsCache.set(cacheKey, result);
        return result;
    },
    ...
    setLang(code) {
        ...
        this.current = code;
        _clearLogDetailsCache(); // Limpa cache de tradução de logs ao trocar idioma
        try {
        ...
        if (typeof Data !== 'undefined' && Data.updateSetting) {
            try { Data.updateSetting('lang', code); } catch (e) { console.warn(e); }
        }
        location.reload();
    }
};
