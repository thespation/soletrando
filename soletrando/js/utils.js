...
const Utils = {
    // Chave de "hoje" (ex: "2026-8-16"), usada tanto para o sorteio
    // determinístico quanto para saber se o Desafio do Dia já foi concluído.
    todayKey() {
        const hoje = new Date();
        return `${hoje.getFullYear()}-${hoje.getMonth()}-${hoje.getDate()}`;
    },

    // Log de depuração (só imprime quando DEBUG = true no topo deste arquivo)
    log(...args) {
        if (DEBUG) console.log(...args);
    },

    // Dicionário de ícones (paths do Lucide) usados em várias telas do admin.
    // Evita repetir o mesmo SVG dezenas de vezes espalhado pelo código.
    _ICONS: {
    ...
    // Monta um ícone SVG a partir do dicionário acima.
    // extraClass é opcional (ex: "icon-lucide") para manter o CSS existente.
    ...
    // Botão-ícone padrão de "remover linha" usado nas tabelas de importação em
    // lote (série, disciplina, palavra). Centraliza o SVG repetido em vários
    // lugares do admin.js. `label` já deve vir traduzido (via T()).
