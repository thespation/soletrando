...
// Ative para reativar os logs de depuração ([IMG], [OPFS], [Export], etc.)
const DEBUG = false;

// Versão do app, mostrada no Painel do Professor → Configurações.
// Atualize aqui a cada mudança relevante — é a única fonte de verdade.
const APP_VERSION = '1.1.2';

/**
 * @typedef {Object} Utils
 * @property {function(): string} todayKey - Retorna chave de data (ex: "2026-8-16")
 * @property {function(...any): void} log - Log condicional (DEBUG)
 * @property {function(string, number, string): string} icon - Gera SVG de ícone
 * @property {function(string, number, string): string} removeRowButton - Botão remover linha
 * @property {function(): string} generateId - UUID v4 ou fallback
 * @property {function(number): string} formatTime - Segundos para MM:SS
 * @property {function(string): string} formatDate - Data para locale
 * @property {function(string): string} formatDateTime - Data/hora para locale
 * @property {function(any[]): any[]} shuffleArray - Fisher-Yates shuffle
 * @property {function(string): string} normalizeLetter - Remove acentos, maiúscula
 * @property {function(string, string): string} capitalizeLetter - Capitalização
 * @property {function(File): Promise<string>} fileToBase64 - File para base64
 * @property {function(string, string): Promise<File>} dataUrlToFile - DataURL para File
 * @property {function(string, string, HTMLElement): void} createElement - Cria elemento
 * @property {function(string): string} escapeHtml - Escapa HTML
 * @property {function(Object): string} logErrorCell - Detalhe de erro para tabelas
 * @property {function(Function, number): Function} debounce - Debounce
 * @property {function(string): Promise<boolean>} copyToClipboard - Clipboard API
 * @property {function(string, string, string): void} downloadFile - Download blob
 * @property {function(string): Promise<File|null>} downloadImageAsFile - Baixa imagem
 * @property {function(string): boolean} confirm - window.confirm
 * @property {function(string): void} alert - window.alert
 * @property {function(HTMLInputElement, Function, number): void} debounceInput - Debounce input
 * @property {function(HTMLElement, HTMLElement): void} scrollTo - Scroll suave
 * @property {function(HTMLElement, number): void} createConfetti - Confetti
 * @property {function(number, number, HTMLElement): void} createSparkle - Sparkle
 * @property {function(): Promise<FileSystemDirectoryHandle|null>} _getImagesRoot - OPFS root
 * @property {function(string, string, string, File): Promise<string|null>} saveImageToDisk - Salva imagem
 * @property {function(string, string, string): Promise<void>} deleteImageFromDisk - Deleta imagem
 * @property {function(string): string} sanitizeFileName - Sanitiza nome arquivo
 * @property {function(string): Promise<string|null>} resolveImagePath - Resolve disk:
 * @property {function(string): Promise<Uint8Array|null>} resolveImageBytes - Resolve bytes
 * @property {function(Object[]): Blob} createZip - Cria ZIP
 * @property {function(ArrayBuffer): Promise<Object[]>} readZip - Lê ZIP
 * @property {function(): Promise<Object[]>} collectAllImagesFromOPFS - Coleta imagens
 * @property {function(Object[]): Promise<void>} importImagesToOPFS - Importa imagens
 * @property {function(FileSystemDirectoryHandle, string): Promise<Object[]>} walkDirectory - Percorre dir
 * @property {function(FileSystemDirectoryHandle, string, any): Promise<void>} writeFileToDir - Escreve arquivo
 * @property {function(FileSystemDirectoryHandle, string): Promise<boolean>} fileExists - Verifica arquivo
 */

const Utils = {
    /**
     * Retorna chave de "hoje" (ex: "2026-8-16") para sorteio determinístico
     * e verificação do Desafio do Dia.
     * @returns {string}
     */
    todayKey() {
        const hoje = new Date();
    ...
    /**
     * Log de depuração (só imprime quando DEBUG = true).
     * @param {...any} args
     */
    log(...args) {
    ...
    // Dicionário de ícones (paths do Lucide)
    _ICONS: {
    ...
    /**
     * Monta um ícone SVG a partir do dicionário.
     * @param {string} name - Nome do ícone
     * @param {number} [size=14] - Tamanho em px
     * @param {string} [extraClass=''] - Classe CSS extra
     * @returns {string} HTML do SVG
     */
    icon(name, size = 14, extraClass = '') {
        const inner = this._ICONS[name];
        if (!inner) return '';
        const cls = extraClass ? ` class="${extraClass}"` : '';
        return `<svg${cls} width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">${inner}</svg>`;
    },

    /**
     * Botão-ícone padrão de "remover linha" para tabelas de importação em lote.
     * @param {string} label - Texto do tooltip/aria-label (já traduzido)
     * @param {number} [size=14]
     * @param {string} [action='remove-row']
     * @returns {string} HTML do botão
     */
    removeRowButton(label, size = 14, action = 'remove-row') {
        return `<button class="btn-icon-sm danger" data-action="${action}" title="${this.escapeHtml(label)}" aria-label="${this.escapeHtml(label)}">
            ${this.icon('x', size)}
        </button>`;
    },

    /**
     * Gera ID único (UUID v4 ou fallback).
     * @returns {string}
     */
    generateId() {
        ...
        // Fallback para contextos sem crypto.randomUUID (ex: http:// não seguro)
    ...
    /**
     * Formata data para locale atual.
    ...
    // Escapar HTML (seguro tanto para conteúdo de texto quanto para uso dentro de
    // atributos como value="${...}" — por isso também escapamos aspas)
    escapeHtml(text) {
    ...
    async downloadImageAsFile(url) {
        ...
        for (const attempt of attempts) {
            try {
                Utils.log('[IMG] Baixando imagem via', attempt.label);
    ...
    async collectAllImagesFromOPFS() {
        ...
        try {
            ...
            Utils.log('[OPFS] Diretório img obtido');
            ...
            try {
                ...
                Utils.log('[OPFS] Imagens da pasta antiga "imagens" migradas');
            } catch (e) { console.warn(e); }
            Utils.log(`[OPFS] ${images.length} imagens coletadas`);
        } catch (e) {
        ...
        try {
            ...
            if (root) {
                ...
                Utils.log(`[FS] ${sysImages.length} imagens da pasta do sistema coletadas`);
            }
        } catch (e) {
...
Utils.APP_VERSION = APP_VERSION;

// Exportar para uso global (navegador) ou como módulo (testes automatizados em Node)
if (typeof window !== 'undefined') {
    window.Utils = Utils;
}
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { Utils, DEBUG };
}