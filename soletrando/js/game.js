 ...
 * @property {string|null} serieId
 * @property {string[]} turmaIds
 * @property {string[]} disciplinaIds
 * @property {string} studentName
 * @property {number|null} palavrasPorTurma
 * @property {'total'|'por'} palavrasModo
 * @property {'sequencial'|'intercalado'} modoOrdem
 * @property {Object|null} currentWord
 * @property {number} currentIndex
 * @property {number[]} revealedLetters
 * @property {string[]} errorLetters
 * @property {number} correctCount
 * @property {number} wrongCount
 * @property {number|null} startTime
 * @property {number} elapsedSeconds
 * @property {number|null} timerInterval
 * @property {Object[]} wordPool
 * @property {Object[]} usedWords
 * @property {string|null} eventId
 * @property {string} eventNome
 * @property {Object[]} wordResults
 * @property {number} streak
 * @property {number} bestStreak
 * @property {boolean} isDailyChallenge
 * @property {string[]} palavrasExcluidas
 * @property {string[]} palavrasSelecionadas
 */
 ...
 * @property {string[]} [palavrasExcluidas]
 * @property {string[]} [palavrasSelecionadas]
 * @property {string|null} [execucaoId]
 */
...
const Game = {
    ...
    start(config) {
        this.state = {
            ...
            isDailyChallenge: config.isDailyChallenge || false,
        ...
        // Zerar badge de sequência de acertos de uma sessão anterior
    ...
    loadWordPool() {
        ...
        // Reordenar palavras: as usadas recentemente vão para o final
        allWords = this._prioritizeNewWords(allWords);
        ...
        if (ppt && numTurmas > 0) {
            ...
            const palavras = [];
            const turmaIdxOrdered = [];
            for (let i = displayOrder.length - 1; i >= 0; i--) {
                palavras.push(displayOrder[i].word);
                turmaIdxOrdered.push(displayOrder[i].turmaIdx);
            }
            // Palavras que não entraram na distribuição inicial ficam como
            // reserva — só são sorteadas se o professor clicar em "+1 Palavra".
            // Sem isso, o pool acaba exatamente no limite configurado e o
            // botão nunca tem palavra nova pra oferecer.
            const usedIds = new Set(palavras.map(p => p.id));
            const leftovers = Utils.shuffleArray(allWords.filter(w => !usedIds.has(w.id)));
            this.state.wordPool = [...leftovers, ...palavras];
            this.state._wordPoolTurmaIdx = [...leftovers.map(() => 0), ...turmaIdxOrdered];
        } else if (ppt) {
            const byDisc = {};
            allWords.forEach(p => {
                if (!byDisc[p.disciplinaId]) byDisc[p.disciplinaId] = [];
            ...
            // Mesma reserva de palavras extras que sobraram, para o "+1 Palavra".
            const shuffledPool = Utils.shuffleArray(pool);
            const usedIds = new Set(shuffledPool.map(p => p.id));
            const leftovers = Utils.shuffleArray(allWords.filter(w => !usedIds.has(w.id)));
            this.state.wordPool = [...leftovers, ...shuffledPool];
    ...
    // Reordenar palavras: as NÃO usadas recentemente vêm primeiro
    // As usadas recentemente vão para o final da fila
    _prioritizeNewWords(words) {
        if (!this.state.disciplinaIds?.length) return words;
        ...
        for (const [discId, pool] of Object.entries(byDisc)) {
            const recentIds = new Set(Data.getRecentWordIds(discId, 30));
            
            // Separar: novas primeiro, recentes por último
            const novas = pool.filter(p => !recentIds.has(p.id));
            const recentes = pool.filter(p => recentIds.has(p.id));
            
            // Embaralhar cada grupo separadamente
            result.push(...Utils.shuffleArray(novas));
            result.push(...Utils.shuffleArray(recentes));
        }
    ...
    handleWrongLetter(letter, expectedLetter) {
        ...
        if (settings.revealWord && errorWordReveal) {
            errorWordReveal.innerHTML = `${T('Palavra:')} <span>${Utils.escapeHtml(this.state.currentWord.texto)}</span>`;
        ...
        // Registrar no histórico de palavras usadas
        Data.addWordToHistory(this.state.currentWord.disciplinaId, this.state.currentWord.id);
    ...
    // Atualiza o badge de sequência de acertos (streak) no cabeçalho do jogo
    updateStreakBadge() {
        ...
        if (this.state.streak >= 3) {
            countEl.textContent = this.state.streak;
            badge.classList.remove('hidden');
            badge.classList.remove('streak-pop');
            void badge.offsetWidth; // reinicia a animação mesmo em streaks seguidas
    ...
    handleWordComplete() {
        ...
        this.state.bestStreak = Math.max(this.state.bestStreak || 0, this.state.streak);
        this.updateStreakBadge();
...
resultado: 'acerto',
    ...
    updateWordCount() {
        ...
        const remainingTextEl = document.getElementById('word-count-remaining-text');
        if (!remainingEl || !remainingTextEl) return;

        if (settings.showRemaining) {
            const remaining = this.state.wordPool.length;
            remainingTextEl.textContent = T('Palavras restantes: {n}', { n: remaining });
    ...
    // Inicia uma nova sessão de prática só com as palavras erradas da sessão anterior
    retryWrongWords() {
        const wrongIds = [...new Set(
            this.state.wordResults.filter(wr => wr.resultado === 'erro' && wr.id).map(wr => wr.id)
        )];
        ...
        const config = {
            ...
            turmaIds: this.state.turmaIds,
            turmaNames: this.state.turmaNames,
            disciplinaIds: this.state.disciplinaIds,
            studentName: this.state.studentName || '',
            modoOrdem: 'sequencial',
            isPractice: true, // não polui estatísticas — é treino das palavras já erradas
            eventId: null,
            eventNome: '',
            palavrasSelecionadas: Utils.shuffleArray(wrongIds)
        };

        App.showScreen('game');
        App.updateGameInputUI();
        this.start(config);
    },

    // Calcula conquistas da sessão atual (sempre baseado só na sessão em si —
    // de propósito não usa dados entre sessões, já que o app roda num
    // computador compartilhado por vários alunos diferentes).
    _computeSessionBadges(total, acertos, erros, taxa) {
        const badges = [];
        if (total >= 3 && erros === 0) {
            badges.push({ icon: '🎯', label: T('Perfeito!') });
        }
        if ((this.state.bestStreak || 0) >= 5) {
            badges.push({ icon: '🔥', label: T('Sequência de Ouro') });
        }
        if (total >= 15) {
            badges.push({ icon: '💪', label: T('Maratona') });
        }
        if (total >= 5 && taxa >= 80 && erros > 0) {
            badges.push({ icon: '🌟', label: T('Mandou Bem') });
        }
        return badges;
    },

    // Renderiza as conquistas da sessão no resultado (esconde a seção se não houver nenhuma)
    _renderSessionBadges(total, acertos, erros, taxa) {
        const container = document.getElementById('session-badges');
        if (!container) return;

        const badges = this._computeSessionBadges(total, acertos, erros, taxa);
        if (badges.length === 0) {
        ...
        container.innerHTML = badges.map(b => `
            <span class="session-badge-chip">
                <span class="session-badge-icon">${b.icon}</span>
                <span class="session-badge-label">${Utils.escapeHtml(b.label)}</span>
            </span>`).join('');
    ...
    showSessionResults() {
        ...
        // Desafio do Dia: só marca como concluído (some da home) se acertou
        // TODAS as palavras — qualquer erro e ele continua disponível.
        if (this.state.isDailyChallenge && total > 0 && erros === 0) {
            Data.markDailyChallengeDone(Utils.todayKey());
        }
        ...
        // Melhor sequência de acertos (só mostra se valer a pena comemorar)
        const streakPill = document.getElementById('session-streak-pill');
        if (streakPill) {
            if ((this.state.bestStreak || 0) >= 3) {
                document.getElementById('session-best-streak').textContent = this.state.bestStreak;
        ...
        // Botão "Praticar as que errei" — só aparece se houve erro
        const retryBtn = document.getElementById('btn-session-retry-wrong');
        if (retryBtn) {
            retryBtn.classList.toggle('hidden', erros === 0);
        }

        // Conquistas da sessão
        this._renderSessionBadges(total, acertos, erros, taxa);
...
const EventGame = {
    ...
    bindEvents() {
        ...
        document.getElementById('screen-event-game')?.addEventListener('click', (e) => {
            ...
            try {
                ...
                switch (action) {
                    ...
                    case 'iniciar-evento': this.iniciarEvento(); break;
                    case 'select-participante': this.selectParticipante(parseInt(btn.dataset.index, 10)); break;
                    case 'toggle-custom-name': this.toggleCustomNameInput(); break;
                    case 'start-student': this.startStudentGame(btn.dataset.nome); break;
                    case 'start-student-from-input': this.startStudentFromInput(); break;
                    case 'go-back-rodada': this.goBackFromRodada(); break;
                    case 'skip-round': this.skipRound(); break;
                    case 'show-phase': this.showPhase(btn.dataset.phase); break;
                    case 'proxima-rodada': this.proximaRodada(); break;
                    case 'confirmar-transicao': this.confirmarTransicao(); break;
                    case 'view-event-results':
                        App.showEventResults(this.state.evento.id, this.state.execucaoId || null, this.state.evento.nome);
                        break;
                }
            } catch (err) {
                console.error('Erro na delegação de eventos (EventGame):', err);
            }
        });
    },
    ...
    renderResumo() {
        ...
        el.innerHTML = `
            <div class="game-header">
                <div class="game-info">
                    <button class="btn-icon" data-action="go-home" title="${T('Voltar')}" aria-label="${T('Voltar')}"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg></button>
                </div>
                <div class="game-header-center">
                    <span class="serie-name">${Utils.escapeHtml(ev.nome)}</span>
                </div>
                <div class="game-controls">
                    <button id="btn-event-theme" class="btn-icon" data-action="toggle-theme" title="${T('Alternar tema')}" aria-label="${T('Alternar tema')}"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="4"/><path d="M12 2v2"/><path d="M12 20v2"/><path d="m4.93 4.93 1.41 1.41"/><path d="m17.66 17.66 1.41 1.41"/><path d="M2 12h2"/><path d="M20 12h2"/><path d="m6.34 17.66-1.41 1.41"/><path d="m19.07 4.93-1.41 1.41"/></svg></button>
                </div>
            </div>
            <div class="event-phase-body event-resumo">
                ...
                ${warningsHtml}
                <button class="btn btn-primary btn-large event-start-btn event-start-btn-iniciar" ${!temAlguma ? 'disabled' : ''} data-action="iniciar-evento">
    ...
    iniciarEvento() {
        ...
        this.state.showCustomNameInput = false;
    ...
    renderRodada() {
        ...
        const showCustom = this.state.showCustomNameInput || participantes.length === 0;
        ...
        if (participantes.length > 0) {
            contentHtml = `
                ...
                <div class="event-participantes-grid ${showCustom ? 'hidden' : ''}">
                    ${participantes.map((p, i) => {
                        const sel = i === this.state.participanteIdx;
                        return `
                        <button class="event-participante-btn ${sel ? 'selected' : ''}" data-action="select-participante" data-index="${i}">
                ...
                <button class="btn-confirm-event ${showCustom ? 'hidden' : ''}" data-action="start-student" data-nome="${Utils.escapeHtml(participanteAtual?.nome || '')}">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                    ${T('Confirmar e Sortear')}
                </button>
                <button class="event-toggle-custom-link ${showCustom ? 'hidden' : ''}" data-action="toggle-custom-name">
                    ${T('Aluno não está na lista? Digite o nome')}
                </button>
                <div class="event-rodada-custom ${showCustom ? '' : 'hidden'}">
                    ...
                    <button class="btn-custom-arrow" data-action="start-student-from-input" title="${T('Confirmar')}" aria-label="${T('Confirmar')}"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg></button>
                    <button class="event-toggle-custom-link" data-action="toggle-custom-name">${T('← Voltar para a lista de alunos')}</button>
                </div>`;
        } else {
            contentHtml = `
                ...
                <button class="btn-confirm-event large" data-action="start-student-from-input">
                    ${T('Sortear Palavra')}
        ...
        el.innerHTML = `
            <div class="game-header">
                <div class="game-info">
                    <button class="btn-icon" data-action="go-back-rodada" title="${T('Voltar')}" aria-label="${T('Voltar')}"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg></button>
                </div>
            ...
            <div class="event-phase-body event-rodada-layout">
                ...
                <div class="event-rodada-content">
                    ${contentHtml}
                    <button class="event-skip-btn" data-action="skip-round">
                        ${podePular ? T('Pular esta rodada') : T('Encerrar evento')}
    ...
    // Alterna entre a lista de alunos cadastrados e o campo de nome livre
    toggleCustomNameInput() {
        this.state.showCustomNameInput = !this.state.showCustomNameInput;
        this.renderRodada();
        if (this.state.showCustomNameInput) {
            setTimeout(() => document.getElementById('event-custom-name')?.focus(), 50);
        }
    },
    ...
    renderFimRodada() {
        ...
        el.innerHTML = `
            <div class="game-header">
                <div class="game-info">
                    <button class="btn-icon" data-action="show-phase" data-phase="resumo" title="${T('Voltar')}" aria-label="${T('Voltar')}"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg></button>
                </div>
                <div class="game-header-center">
                    <span class="serie-name">${Utils.escapeHtml(ev.nome)}</span>
                </div>
            ...
            <div class="event-phase-body event-rodada-layout">
                ...
                <div class="event-stats-card event-stats-table">
                    ...
                    <div class="event-stat">
                        <div class="event-stat-icon">${Utils.icon('clock', 28)}</div>
                        <p class="event-stat-label">${T('Tempo')}</p>
                        <p class="event-stat-value">${Utils.formatTime(resumo.tempoTotal)}</p>
                    </div>
                </div>
                ${wordListHtml}
                <button class="btn btn-primary btn-large event-start-btn" data-action="proxima-rodada">
    ...
    proximaRodada() {
        try {
            ...
            Utils.log('EventGame.proximaRodada:', { rodadaIdx: this.state.rodadaIdx, proximaIdx, totalRodadas: rodadas.length, idAtual });
    ...
    renderTransicao() {
        ...
        el.innerHTML = `
            <div class="game-header">
                <div class="game-info">
                    <button class="btn-icon" data-action="show-phase" data-phase="fim_rodada" title="${T('Voltar')}" aria-label="${T('Voltar')}"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg></button>
                </div>
                <div class="game-header-center">
                    <span class="serie-name">${Utils.escapeHtml(ev.nome)}</span>
                </div>
            ...
            <div class="event-phase-body event-center">
                ...
                <button class="btn btn-primary btn-large event-start-btn" data-action="confirmar-transicao">
                    ${T('Continuar')}
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="inline-icon"><path d="m9 18 6-6-6-6"/></svg>
                </button>
            </div>`;
        ...
        el.innerHTML = `
            ...
            <div class="event-phase-body event-center">
                ...
                <div class="event-transition-btns" style="flex-direction:column">
                    <button class="btn btn-primary btn-large event-start-btn" data-action="view-event-results">
                    ...
                    <button class="btn btn-secondary" data-action="go-home">${T('Voltar ao Início')}</button>
                </div>
            </div>`;
    },
    ...
    skipRound() {
        const isLast = this.proximaRodadaComPalavras(this.state.rodadaIdx + 1) === -1;
        const msg = isLast
            ? T('Encerrar o evento agora? Isso não pode ser desfeito.')
            : T('Pular esta rodada? Ela não poderá ser jogada depois — essa ação não pode ser desfeita.');
