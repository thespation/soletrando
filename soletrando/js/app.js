...
const App = {
    ...
    bindEvents() {
        ...
        document.querySelectorAll('.palette-btn').forEach(btn => {
            btn.addEventListener('click', () => this.setPalette(btn.dataset.palette));
        });
        ...
        document.getElementById('btn-session-retry-wrong')?.addEventListener('click', () => Game.retryWrongWords());
        ...
        document.getElementById('app')?.addEventListener('click', (e) => {
            ...
            try {
                ...
                switch (action) {
                    ...
                    case 'start-daily-challenge': this.startDailyChallenge(el.dataset.serieId, el.dataset.disciplinaId); break;
                }
            } catch (err) {
    ...
    // Monta o card do "Desafio do Dia" na home — mesma escolha o dia todo,
    // sorteada de forma determinística a partir da data (sem estado salvo).
    loadDailyChallenge() {
        ...
        // Recurso desligado nas Configurações
        const settings = Data.getSettings();
        if (settings.dailyChallengeEnabled === false) return;

        // Já foi concluído hoje com 100% de acerto — não mostra de novo
        const todayKey = Utils.todayKey();
        const state = Data.getDailyChallengeState();
        if (state && state.done && state.date === todayKey) return;

        let series, palavras;
        try {
            series = Data.getSeries().filter(s => s.active !== false);
            palavras = Data.getPalavras();
        } catch (e) { return; }

        // Só considera combinações de série+disciplina com pelo menos 3 palavras
        const candidatos = [];
        series.forEach(s => {
            const disciplinas = Data.getDisciplinasBySerie(s.id);
            disciplinas.forEach(d => {
                const count = palavras.filter(p => p.serieId === s.id && p.disciplinaId === d.id).length;
                if (count >= 3) candidatos.push({ serie: s, disciplina: d });
            });
        });
        if (candidatos.length === 0) return;

        // Seed determinístico baseado na data de hoje — mesma escolha o dia
        // inteiro pra quem abrir o app, mas muda todo dia.
        let seed = 0;
        ...
        banner.className = 'home-daily-challenge';
        banner.innerHTML = `
            <div class="daily-challenge-icon">🎯</div>
            <div class="daily-challenge-info">
                <strong data-i18n="Desafio do Dia">${T('Desafio do Dia')}</strong>
                <span>${Utils.escapeHtml(escolhido.serie.nome)} · ${Utils.escapeHtml(escolhido.disciplina.nome)}</span>
            </div>
            <button class="btn btn-accent btn-small" data-action="start-daily-challenge" data-serie-id="${escolhido.serie.id}" data-disciplina-id="${escolhido.disciplina.id}">${T('Jogar')}</button>`;

        const ref = homeContainer.querySelector('.home-links');
        homeContainer.insertBefore(banner, ref);
    },

    // Inicia o jogo direto com a série/disciplina do Desafio do Dia, pulando a
    // tela de seleção — um atalho de "um toque" pra começar a jogar.
    startDailyChallenge(serieId, disciplinaId) {
    ...
    loadTheme() {
        ...
        this.applyPalette(settings.palette || 'nord');
        this.updateThemeToggleIcon();
    },

    // Paletas disponíveis (correspondem aos [data-palette] em base.css).
    // "nord" é o padrão e não precisa do atributo (ver base.css).
    PALETTES: ['nord', 'catppuccin'],

    // Aplica a paleta de cores escolhida (independente de claro/escuro,
    // que continua controlado por [data-theme] via toggleTheme())
    applyPalette(palette) {
        if (palette && this.PALETTES.includes(palette) && palette !== 'nord') {
    ...
    // Salva e aplica a nova paleta escolhida nas Configurações
    setPalette(palette) {
        Data.updateSetting('palette', palette);
...
loadResultsPage(eventoId, execucaoId, eventName) {
    ...
    renderDisciplineChart(containerId, logs) {
        ...
        const colors = ['rgba(var(--primary-rgb), 0.2)', 'rgba(var(--accent-rgb), 0.2)'];
        const colorsSolid = ['var(--primary)', 'var(--accent)'];
    ...
    drawBarChart(canvasId, logs) {
        ...
        // Canvas não entende var(--x) — resolve as cores reais do tema atual.
        const rootStyle = getComputedStyle(document.documentElement);
        const primaryRgb = rootStyle.getPropertyValue('--primary-rgb').trim() || '34, 197, 94';
        const accentRgb = rootStyle.getPropertyValue('--accent-rgb').trim() || '245, 158, 11';
        const primarySolid = rootStyle.getPropertyValue('--primary').trim() || '#22c55e';
        const accentSolid = rootStyle.getPropertyValue('--accent').trim() || '#f59e0b';
        const colors = [`rgba(${primaryRgb}, 0.55)`, `rgba(${accentRgb}, 0.55)`];
        const colorsBorder = [primarySolid, accentSolid];
