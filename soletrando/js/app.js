...
const App = {
    ...
    loadDailyChallenge() {
        ...
        const hoje = new Date();
        const seedStr = `${hoje.getFullYear()}-${hoje.getMonth()}-${hoje.getDate()}`;
        let seed = 0;
    ...
    loadTheme() {
        ...
        this.applyColorTheme(settings.theme || 'default');
        this.updateThemeToggleIcon();
    },

    // Cores disponíveis para o tema (correspondem aos [data-color] em base.css)
    COLOR_THEMES: ['default', 'ocean', 'forest', 'sunset', 'purple', 'rose'],

    // Aplica a cor de destaque escolhida (não mexe em claro/escuro)
    ...
    // Salva e aplica a nova cor de destaque escolhida nas Configurações
    setColorTheme(theme) {
        Data.updateSetting('theme', theme);
        this.applyColorTheme(theme);
        document.querySelectorAll('.color-theme-swatch').forEach(el => {
