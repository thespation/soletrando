...
/**
 * Camada de persistência e lógica de dados do Soletrando.
 * Usa localStorage como backend principal, com suporte a File System Access API (OPFS)
 * para imagens e backups zip.
 * @namespace Data
 */
const Data = {
    /** @type {Object.<string, string>} Chaves do localStorage por entidade */
    KEYS: {
    ...
    /** @type {number} Teto de logs de partida para evitar estouro de quota (~5MB) */
    MAX_LOGS: 2000,
    /** @type {number} Teto de logs de atividade do professor */
    MAX_ADMIN_LOGS: 500,

    /**
     * Grava no localStorage com tratamento de erro.
     * @param {string} key - Chave do localStorage
     * @param {any} value - Valor a serializar (JSON)
    ...
    // ===== DESAFIO DO DIA =====
    // Guarda se o Desafio do Dia de hoje já foi concluído com 100% de acerto
    // (nesse caso ele some da home até o dia seguinte).
    /**
     * Obtém estado do Desafio do Dia.
     * @returns {{date: string, done: boolean}|null}
     */
    getDailyChallengeState() {
        try {
            const raw = localStorage.getItem(this.KEYS.DAILY_CHALLENGE);
    ...
    /**
     * Marca Desafio do Dia como concluído.
     * @param {string} dateKey - Chave de data (ex: "2026-1-15")
     */
    markDailyChallengeDone(dateKey) {
        this._persist(this.KEYS.DAILY_CHALLENGE, { date: dateKey, done: true });
    },

    // ===== HISTÓRICO DE PALAVRAS =====
    // Rastreia palavras usadas recentemente por disciplina para evitar repetição
    /**
     * Obtém histórico completo de palavras por disciplina.
     * @returns {Object.<string, string[]>}
     */
    getWordHistory() {
        try {
            const raw = localStorage.getItem(this.KEYS.WORD_HISTORY);
    ...
    /**
     * Salva histórico de palavras.
     * @param {Object.<string, string[]>} history
     */
    _saveWordHistory(history) {
        this._persist(this.KEYS.WORD_HISTORY, history);
    },

    /**
     * Retorna IDs das palavras usadas recentemente para uma disciplina.
     * @param {string} disciplinaId
     * @param {number} [limit=30]
     * @returns {string[]}
     */
    getRecentWordIds(disciplinaId, limit = 30) {
        const history = this.getWordHistory();
        const key = `disc_${disciplinaId}`;
        return history[key]?.slice(-limit) || [];
    },

    /**
     * Adiciona uma palavra ao histórico de uso (move para o final se já existe).
     * @param {string} disciplinaId
     * @param {string} palavraId
     * @param {number} [maxSize=50]
     */
    addWordToHistory(disciplinaId, palavraId, maxSize = 50) {
        const history = this.getWordHistory();
        const key = `disc_${disciplinaId}`;
        if (!history[key]) history[key] = [];
        
        // Remove se já existe (para mover para o final)
        history[key] = history[key].filter(id => id !== palavraId);
        
        // Adiciona no final
        history[key].push(palavraId);
        
        // Limita tamanho
        if (history[key].length > maxSize) {
            history[key] = history[key].slice(-maxSize);
        }
        
        this._saveWordHistory(history);
    },

    /**
     * Adiciona múltiplas palavras ao histórico (ao final de uma partida).
     * @param {string} disciplinaId
     * @param {string[]} palavraIds
     * @param {number} [maxSize=50]
     */
    addWordsToHistory(disciplinaId, palavraIds, maxSize = 50) {
        if (!palavraIds.length) return;
        const history = this.getWordHistory();
        const key = `disc_${disciplinaId}`;
        if (!history[key]) history[key] = [];
        
        for (const id of palavraIds) {
            history[key] = history[key].filter(existing => existing !== id);
            history[key].push(id);
        }
    ...
    // ===== SÉRIES =====
    /**
     * Obtém todas as séries (com migração de legacy).
     * @returns {Array<{id:string,nome:string,professor:string,active:boolean,createdAt:string}>}
     */
    getSeries() {
    ...
    /**
     * Cria nova série.
     * @param {{nome:string, professor?:string}} series
     * @returns {{id:string,nome:string,professor:string,active:boolean,createdAt:string}}
     */
    addSeries(series) {
    ...
    addAdminLog(action, section, details) {
        ...
        if (logs.length > this.KEYS.MAX_ADMIN_LOGS) {
            logs.splice(0, logs.length - this.KEYS.MAX_ADMIN_LOGS);
        }
    ...
    addLog(log) {
        ...
        all.push(newLog);
        // Descarta os logs mais antigos além do teto (MAX_LOGS).
        if (all.length > this.KEYS.MAX_LOGS) {
            all.splice(0, all.length - this.KEYS.MAX_LOGS);
        }
        this.saveLogs(all);
        return newLog;
    },
    ...
    updateSetting(key, value) {
        ...
        if (oldVal !== value && key !== 'darkMode' && key !== 'palette') {
            const labels = {
                darkMode: 'Tema',
                ...
                lang: 'Idioma',
                dailyChallengeEnabled: 'Desafio do Dia'
            };
    ...
    async connectSystemFolder(forcePicker = false) {
        ...
        try {
            ...
            // showDirectoryPicker falha em modo privado/incógnito — capturar erro específico
            let root;
            try {
                root = await this._findProjectRoot(await window.showDirectoryPicker({ mode: 'readwrite' }));
            } catch (e) {
                // Modo incógnito/privado ou usuário cancelou
                const msg = e?.message || '';
                if (msg.includes('incognito') || msg.includes('private') || msg.includes('Incognito') || msg.includes('Private') || e?.name === 'SecurityError' || e?.name === 'NotAllowedError') {
                    alert(T('Não é possível conectar a pasta no modo navegação privada/incógnito. Abra o app em uma janela normal do navegador.'));
                }
                return null;
            }
            ...
            console.warn('Falha ao conectar pasta do sistema:', e);
            return null;
        }
    },
